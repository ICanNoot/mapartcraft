// Bottom status bar — cursor position, zoom, canvas dimensions
      ...
      {inBounds ? (
        <>
          ...
          <div className="status-item">
            ...
            <span className="status-value">{mapX},{mapY}</span>
          </div>
        </>
      ) : (
        <div className="status-item">
          <span className="status-label">{project ? 'Cursor outside canvas' : 'No project'}</span>
        </div>
      )}
      <div style={{ flex: 1 }} />
      <div className="status-item">
